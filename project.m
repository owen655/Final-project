...
%	a) Equilibrium Glide at Maximum Lift/Drag Ratio
	...
	xoNom	=	[3.55;Gam;H;R];   % Sets nominal values
	[ta,xa]	=	ode23('EqMotion',tspan,xoNom);
    xoLow   =   [2;Gam;H;R];       % Sets Low velocity
    [ta,xb]	=	ode23('EqMotion',tspan,xoLow);
    xoHigh  =   [7.5;Gam;H;R];     % Sets High velocity
    [ta,xc]	=	ode23('EqMotion',tspan,xoHigh);
	
	
	figure           % Plots variations from changing velocity
    subplot(2,1,1)  
...
% Sets values for nominal, low, and high starting flight path angle
    ...
    % Plots variations for changing flight path angle
    ...
    % Generates random velocity and flight path angle within range and
    % plots them. Does this 100 time
    ...
    polyOrder = 6;  % Order of polynomial fit
    

    % Creates polynomial fit
    ...
    % Plots polynomial fit over random generated parameter plots
    ...
    % Plots range vs time of polynomial fit curve 
    ...
    % Plots height vs time of polynomial fit curve
    ...
    deltaRange = diff(yRange);  %Find change in range of adjacent range values
    deltaHeight = diff(yHeight); %Finds change in height of adjacent height values
    deltaT = diff(tspan);  %Finds change in time


    % Finds and plots change in range with respect to time vs time
    ...
    % Finds and plots change in height with respect to time vs time
